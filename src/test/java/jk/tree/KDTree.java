 ...
 ** 
 ** Fix in ballSearch: use squared radius, by Ilari Vallivaara 2020-09-26
 ** 
...
public abstract class KDTree<T> {
    ...
    public ArrayList<T> ballSearch(double[] searchLocation, double radius) {
        final double radiusSq = sqr(radius);
        IntStack stack = new IntStack();
        ...
        while (stack.size() > 0) {
            int nodeIndex = stack.pop();
            if (radiusSq > pointRectDist(nodeIndex, searchLocation)) {
                Node node = nodeList.get(nodeIndex);
                if (node.pointLocations == null)
                    stack.push(node.moreIndex).push(node.lessIndex);
                else
                    node.searchBall(searchLocation, radiusSq, results);
            }
        }
        return results;
    }
