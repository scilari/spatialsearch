...
final class QuadTree[E <: Position] private (bb: AABB, val parameters: Parameters)
  extends RootedContainer[E] with EuclideanSearches[E] with SeqSearches[E] {
...
object QuadTree{
  ...
  def apply[E <: Position](bb: AABB, points: collection.Seq[E], parameters: Parameters): QuadTree[E] = {
    val tree = QuadTree[E](bb, parameters)
    points.foreach{ p =>
      require(bb.contains(p.position)) // TODO: will the insert be even faster if this is not checked?
      tree.add(p)
    }
    tree
  }
  ...
  def apply[E <: Position](elems: collection.Seq[E]): QuadTree[E] = QuadTree(elems, Parameters.default)
  ...
  import com.scilari.geometry.spatialsearch.quadtree.QuadTreeUtils.{bottomLeftAABB, bottomRightAABB, findQuadrant, topLeftAABB, topRightAABB}
  ...
  final class Branch[E <: Position](
  ...
  ) extends Node[E] with Tree.Branch[E, Node[E]] {
    ...
    def compress(): Unit = {
      ...
      bounds = AABB(children.map{_.bounds}.map{_.corners}.flatten)
    }
  }
  ...
  final class Leaf[E <: Position](
    ...
    val parent: Option[Node[E]] = None,
    parameters: Parameters
  ) extends Node[E] with Tree.Leaf[E, Node[E]]{
    ...
    def compress(): Unit = {
      bounds = AABB(elements.map{_.position})
    }
  }
