...
object ArrayUtils {
  ...
  def meanAngle(a: Array[Float]): Float = {
    val sumX = a.map{math.cos}.sum
    val sumY = a.map{math.sin}.sum
  ...
  def weightedMeanAngle(a: Array[Float], w: Array[Float]): Float = {
    val ys = a.map{math.sin}
    val xs = a.map{math.cos}
    val wys = (ys, w).zipped.map(_*_)
    val wxs = (xs, w).zipped.map(_*_)
    val sumX = wxs.sum
    val sumY = wys.sum
    if(sumX == 0 && sumY == 0) a(0) else math.atan2(sumY, sumX)
  }
