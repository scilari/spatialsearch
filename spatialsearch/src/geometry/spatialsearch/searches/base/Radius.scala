...
trait Radius[E <: Position](r: Float) extends DistanceConfig[Float2, E] with Rooted[E] {
  ...
  def search(queryPoint: Float2): collection.Seq[E] = {
    range(queryPoint, List(root), rSq)
  }

  def searchExcludeNode(queryPoint: Float2, node: Node[E]): collection.Seq[E] = {
    rangeExcludeNode(queryPoint, List(root), rSq, node)
  }

  def searchLeaves(queryPoint: Float2): collection.Seq[Node[E]] = {
  ...
  private def range(queryPoint: Float2, rootNodes: List[Node[E]], rSq: Float): collection.Seq[E] = {
    val leaves = rangeLeaves(queryPoint, rootNodes, rSq)
    var foundElems: List[E] = Nil
    leaves.foreach{ leaf =>
      val es = leaf.elements
  ...
  private def rangeLeaves(queryPoint: Float2, rootNodes: List[Node[E]], rSq: Float): collection.Seq[Node[E]] = {
