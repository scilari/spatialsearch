...
final case class MultiTree[E <: Position](roots: Seq[Node[E]])
    extends EuclideanSearches[E]
    with SeqSearches[E] {
  var root: Node[E] = MultiTree.MultiBranch[E](roots.toArray)
}

object MultiTree {
  def fromTrees[E <: Position](trees: Seq[QuadTree[E]]): MultiTree[E] =
    MultiTree[E](trees.map(_.root))
  class MultiBranch[E <: Position](var children: ArraySeq[Node[E]])
      extends Node[E]
