...
/** Optimized version for computing k smallest values from a stream of n >> k values. Skips enqueue
  * operations when the queue is full and the inserted element is larger than the current max.
  * @param k
  *   The number of smallest values we want to compute.
  * @tparam E
