...
class FloatMinHeap[E](initialCapacity: Int = FloatMinHeap.defaultInitialCapacity) {
  ...
  def peekValuesToBuffer(b: Buffer[E]): Buffer[E] = {
    var i = firstIndex
    while(i <= maxIndex){
  ...
  def peekValues: mutable.Buffer[E] = peekValuesToBuffer(Buffer[E]())
  
 
  def getKeysAndValues: mutable.Buffer[FloatKey[E]] = {
    val b = mutable.Buffer[Any]()
    var i = firstIndex
    while(i < maxIndex){  // TODO: check if this is ok
      b += FloatKey(keys(i), values(i))
      i += 1
    }
    b.asInstanceOf[mutable.Buffer[FloatKey[E]]]
  }
