...
class Float3(var x: Float = 0f, var y: Float = 0f, var z: Float = 0f) {
  ...
  def clamp(floor: Float3, ceil: Float3): Float3 =
    new Float3(floatClamp(x, floor.x, ceil.x), floatClamp(y, floor.y, ceil.y), floatClamp(z, floor.z, ceil.z))

  def clampNormalize(floor: Float3, ceil: Float3): Float3 = (clamp(floor, ceil) - floor)/(ceil-floor)
